...
/** biome-ignore-all lint/complexity/useOptionalChain: Third party */
/** biome-ignore-all lint/style/useTemplate: Third party */
/** biome-ignore-all lint/suspicious/noAssignInExpressions: Third party */
/** biome-ignore-all lint/suspicious/noRedeclare: Third party */
/** biome-ignore-all lint/suspicious/noDoubleEquals: Third party */
/** biome-ignore-all lint/style/useBlockStatements: Third party */
/** biome-ignore-all lint/correctness/noInnerDeclarations: Third party */
...
export function parse(S, options) {
   ...
   */
  var SelfClosingTags = options.selfClosingTags ||
    options.noChildNodes || ["img", "br", "input", "meta", "link", "hr"];
  ...
  if (options.simplify) {
    ...
    // @ts-expect-error - simplify returns different type structure
    return simplify(arrOut);
  }

  if (options.setPos && typeof out === "object" && !Array.isArray(out)) {
    // @ts-expect-error - adding pos property dynamically
    out.pos = pos;
  }

  // @ts-expect-error - return type varies based on options
  return out;
}
